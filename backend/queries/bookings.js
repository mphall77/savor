...
const createBooking = async (newBooking) => {
	...
	try {
		const res = await db.one(
			`INSERT INTO bookings ( chef_id, user_id, status, cuisine, event_date, start_time, end_time) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
...
const updateBooking = async (id, booking) => {
	...
	try {
		...
		const updateBooking = await db.one(query, [
			chef_id,
			user_id,
			status,
			cuisine,
